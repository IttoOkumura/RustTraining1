...
// 推理ロジックの実装
pub const GUESS_LENGTH: usize = 5; // 文字列
pub const GUESS_MAX: usize = 6; // 試行最大数
...
mod tests {
    ...
    fn test_guess_count_of_letters() {
        ...
        let spell_guess = super::WordGuess {
            letters: vec![
                GuessLetter { letter: 'g', accuracy: HitAccuracy::InRightPlace},
                GuessLetter { letter: 'u', accuracy: HitAccuracy::InWord},
                GuessLetter { letter: 'e', accuracy: HitAccuracy::InRightPlace},
                GuessLetter { letter: 's', accuracy: HitAccuracy::InWord},
                GuessLetter { letter: 's', accuracy: HitAccuracy::NotInWord},
            ],
        };
        assert_eq!(game.guesses[0], spell_guess);
    }
